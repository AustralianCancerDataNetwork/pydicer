...
def convert_dicom_to_nifty(input_filepaths,
    ...
    if modality == 'PT':
        if hasattr(slices[0], 'PatientWeight') and (slices[0].PatientWeight is None):
            if hasattr(slices[0], 'PatientsWeight'):
                ...
                patient_weight = 75.0  # From Shuchao's code
                warnings.warn(
    ...
    # pang due to an error at line 144: TypeError: only size-1 arrays can be converted to Python scalars
...
def get_physical_values_pt(slices, patient_weight, dtype=np.float32):
    s = slices[0]
    #units = s.Units
    if 'Units' in s: # from Shuchao's code
        units = s.Units
    else:
        units = 'CNTS'
...
def get_suv_philips(slices, dtype=np.float32):
    ...
    for s in slices:
        if suv_scale_factor_tag in s and 'RescaleSlope' in s and 'RescaleIntercept' in s: # # From Shuchao's code
...
def get_suv_from_bqml(slices, decay_time, patient_weight, dtype=np.float32):
    ...
    for s in slices:
        ...
        if 'RadionuclideHalfLife' in s.RadiopharmaceuticalInformationSequence[0]:
          ...
          half_life = 6500.0 # # From Shuchao's code
        ...
        if 'RadionuclideTotalDose' in s.RadiopharmaceuticalInformationSequence[0]:
          ...
          total_dose = 487254240.0 # # From Shuchao's code
