...
def add_object(
    ...
    if object_type not in ["image", "structure", "plan", "dose"]:
        raise ValueError(
            "Object type must be one of: 'image', 'structure', 'plan' or 'dose'"
        )
    ...
    if len(df_converted[df_converted.hashed_uid == object_id]) > 0:
        raise SystemError(
            f"An object with ID {object_id} already exists for this patient!"
        )
    ...
    for dataset in datasets:
        ...
        if not patient_directory.exists():
            logger.warning(
                "Patient %s doesn't exist in dataset %s", patient_id, dataset
            )
            continue
    ...
    if linked_obj is not None:
        if isinstance(linked_obj, str):
            ...
            if not len(df_linked) == 1:
                raise SystemError(
                    f"Expecting one linked_plan object but found {len(df_linked) }"
                )
...
def add_image_object(
    ...
    image_object_path = patient_directory.joinpath(f"{object_type}s", image_id)
    image_object_path.mkdir(exist_ok=True, parents=True)

    # Save the data object in the image_object_path
    image_file = image_object_path.joinpath(f"{modality}.nii.gz")
...
def add_structure_object(
    ...
    # Save the data object in the structure_object_path
    for struct_name, struct_mask in structures.items():
        sitk.WriteImage(
            struct_mask, str(structure_object_path.joinpath(f"{struct_name}.nii.gz"))
        )
