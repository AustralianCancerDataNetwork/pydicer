...
class PatientLogger:
    ...
    def log_module_error(self, module, hashed_uid, error_log, start_time, end_time):
        ...
        df_error = pd.DataFrame(
            [[module, hashed_uid, 1, error_log, start_time, end_time]], columns=SUMMARY_CSV_COLS
        )
        df_error.to_csv(self.summary_csv_path, header=False, mode="a", index=False)

    def eval_module_process(self, module, hashed_uid, start_time, end_time):
        ...
        if len(df_summary_mod) == 0:
            df_final_summary = pd.DataFrame(
                [[module, hashed_uid, 0, "", start_time, end_time]], columns=SUMMARY_CSV_COLS
            )
